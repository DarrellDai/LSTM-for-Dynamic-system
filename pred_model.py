...
parser.add_argument('--seq_length_pred', default=50000, type=int, metavar='T',
...
parser.add_argument('--loss-type', '--lt', default='mixed_s', type=str, metavar='LT',
...
parser.add_argument('--data-file', default='data/baro2_h10sU20_dk1dU1', type=str, metavar='DATA_PATH',
...
fname = 'train_ls4stg0hs{}lag{}nl{}_seq{}nsk{}np{}_epoch{}_'.format(args.nhid, args.nlag,  args.nloss,
...
def main():
    ...
    print(traj_set.size())
    init1 = torch.zeros(args.input_length, 1, 5, dtype=torch.double)
    init2 = torch.zeros(args.input_length, 1, 5, dtype=torch.double)
    ...
    init1[:, 0, 1:3] = torch.from_numpy(samp_w[:args.input_length, :2])
    init2[:, 0, 1:3] = torch.from_numpy(samp_w[:args.input_length, 2:])
    init1[:, 0, 3:] = torch.from_numpy(samp_t[:args.input_length, :2])
    init2[:, 0, 3:] = torch.from_numpy(samp_t[:args.input_length, 2:])
    ...
    for i in range(err_pred.shape[0]):
        err_ave[i] = np.mean(err_pred[max(0,i-10):i+1], axis=0)
        
    datapath = os.path.join(cfg['checkpoint'], 'trajs_sU5_' + fname)
...
def prediction(init_set, targ_set, model, device):
    ...
    with torch.no_grad():
        model1.eval()
        model2.eval()
    baroU = BaroTopo(dt = args.nskip*0.01, H0=1, device=dev1)
    ...
    traj_pred[:args.input_length,5:7] = istate1.data.cpu().numpy()[:args.input_length,0,3:]
    traj_pred[:args.input_length,7:] = istate2.data.cpu().numpy()[:args.input_length,0,3:]
    ...
    with torch.no_grad():
        for start in range(args.input_length, args.seq_length_pred):
            ...
            pred1 = output1.data.cpu().numpy()[-1,0,:]
            pred2 = output2.data.cpu().numpy()[-1,0,:]
            predU = outputU.data.cpu().numpy()[-1,0]
            targ = target.data.cpu().numpy()[start,:9]
            traj_pred[start,[1,2,5,6]] = pred1
            traj_pred[start,[3,4,7,8]] = pred2
            traj_pred[start,0] = predU
            errors[start,0] = np.square(predU - targ[0])
            errors[start,1] = np.square(pred1[:2]-targ[1:3]).sum()
            errors[start,2] = np.square(pred2[:2]-targ[3:5]).sum()
            errors[start,3] = np.square(pred1[2:]-targ[5:7]).sum()
            errors[start,4] = np.square(pred2[2:]-targ[7:]).sum()
            ...
            istate1[:,0,1:] = torch.from_numpy(traj_pred[start-args.input_length+1:start+1,[1,2,5,6]])
            istate2[:,0,1:] = torch.from_numpy(traj_pred[start-args.input_length+1:start+1,[3,4,7,8]])
            inputs[:,0,1:] = torch.from_numpy(traj_pred[start-args.input_length+1:start+1,1:5])
    
            if start % 10 == 0:
                print('step {}: error_v = {:.6f}, error_T = {:.6f}, error_U = {:.6f}'.format(start, errors[start,1:3].sum(), errors[start,3:].sum(), np.square(predU - targ[0])))
