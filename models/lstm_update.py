...
class LSTMupdate(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, nlags, nlayers=4, nstages=0, expan=10):
        ...
        # learnable weights in outputs of size nlags
        lin_wei = []
        for i in range(nlags):
            lin_wei.append(nn.Linear(hidden_size, output_size, bias=False) )
        self.lin_wei = nn.ModuleList(lin_wei)
    ...
    def forward(self, inputs, hidden = (), npred = 0, device = "cpu"):
        ...
        for i in range(inputs.size(0)):            
            ...
            if i < self.nlags:
                ...
                stage[:-1] = stage[1:]
                stage[-1] = hout
            # compute weights for the output combination
            if i > 0:
                refi = inputs[i,:,-self.output_size:]-inputs[i-1,:,-self.output_size:]
            else:
                refi = inputs[i+1,:,-self.output_size:]-inputs[i,:,-self.output_size:]
            wei = torch.zeros(min(self.nlags, i+1), inputs.size(1), dtype=torch.double, device=device)
            for j in range(min(self.nlags, i+1)):
                wei[-j-1,:] = (self.lin_wei[-j-1](stage[-j-1]) * refi).sum(1)
            # output states MUST be stored in the last entries of inputs
            ncomb = (F.softmax(wei, dim=0)[:,:, None].repeat(1,1,self.hidden_size) * \
                     torch.stack(stage, 0)).sum(0)
            #print((torch.stack(stage, 0)).size())
            incre = self.lin_out(ncomb)
